...
def int32(x):
    ...
    return wrapped - half_range  # go back
...
MPC_SEL__NEXT = 2**22
LATCH_MPC = 2**23
JUMP_IF_NEGATIVE = 2**24
JUMP_IF_ZERO = 2**25
JUMP_IF_POSITIVE = 2**26
HALT = 2**27
...
class InstructionDecoder:
    ...
    def micro_code_address(self):
        ...
        return instruction_map[arg_type][opcode_name]
...
class ControlUnit:
    ...
    def data_address_sel(self):
        if self.has_signal(DA_SEL__ARG):
            return 0
        if self.has_signal(DA_SEL__DATA_OUT):
            return 1
        return None
    ...
    def acc_sel(self):
        if self.has_signal(ACC_SEL__ARG):
            return 0
        if self.has_signal(ACC_SEL__INPUT_PORT):
            return 1
        if self.has_signal(ACC_SEL__DATA_OUT):
            return 2
        if self.has_signal(ACC_SEL__ALU):
            return 3
        return None
    ...
    def alu_operand_sel(self):
        if self.has_signal(ALU_OPERAND_SEL__DATA_OUT):
            return 0
        if self.has_signal(ALU_OPERAND_SEL__ARG):
            return 1
        return None
    ...
    def alu_operation_sel(self):
        if self.has_signal(ALU_OPERATION_SEL__ADD):
            return 0
        if self.has_signal(ALU_OPERATION_SEL__SUB):
            return 1
        if self.has_signal(ALU_OPERATION_SEL__MUL):
            return 2
        if self.has_signal(ALU_OPERATION_SEL__DIV):
            return 3
        if self.has_signal(ALU_OPERATION_SEL__MOD):
            return 4
        return None
    ...
    def mpc_sel(self):
        ...
        if self.has_signal(MPC_SEL__NEW):
            return 1
        if self.has_signal(MPC_SEL__NEXT):
            return 2
        return None
...
MICROCODE = [
    ...
    # ld
    ACC_SEL__ARG | LATCH_ACC | LATCH_PC | MPC_SEL__FETCH | LATCH_MPC,
    # add
    ALU_OPERAND_SEL__ARG | ALU_OPERATION_SEL__ADD | ACC_SEL__ALU | LATCH_ACC | LATCH_PC | MPC_SEL__FETCH | LATCH_MPC,
    # sub
    ALU_OPERAND_SEL__ARG | ALU_OPERATION_SEL__SUB | ACC_SEL__ALU | LATCH_ACC | LATCH_PC | MPC_SEL__FETCH | LATCH_MPC,
    # mul
    ALU_OPERAND_SEL__ARG | ALU_OPERATION_SEL__MUL | ACC_SEL__ALU | LATCH_ACC | LATCH_PC | MPC_SEL__FETCH | LATCH_MPC,
    # div
    ALU_OPERAND_SEL__ARG | ALU_OPERATION_SEL__DIV | ACC_SEL__ALU | LATCH_ACC | LATCH_PC | MPC_SEL__FETCH | LATCH_MPC,
    # mod
    ALU_OPERAND_SEL__ARG | ALU_OPERATION_SEL__MOD | ACC_SEL__ALU | LATCH_ACC | LATCH_PC | MPC_SEL__FETCH | LATCH_MPC,
    # all instructions with address argument:
    # ld
    DA_SEL__ARG | LATCH_DA | MPC_SEL__NEXT | LATCH_MPC,
    DATA_READ | ACC_SEL__DATA_OUT | LATCH_ACC | LATCH_PC | MPC_SEL__FETCH | LATCH_MPC,
    # st
    DA_SEL__ARG | LATCH_DA | MPC_SEL__NEXT | LATCH_MPC,
    ...
    DA_SEL__ARG | LATCH_DA | MPC_SEL__NEXT | LATCH_MPC,
    DATA_READ
    | ALU_OPERAND_SEL__DATA_OUT
    | ALU_OPERATION_SEL__ADD
    | ACC_SEL__ALU
    | LATCH_ACC
    | LATCH_PC
    | MPC_SEL__FETCH
    ...
    | LATCH_MPC,
    # jmp
    LATCH_PC | MPC_SEL__FETCH | LATCH_MPC | JUMP_IF_NEGATIVE | JUMP_IF_ZERO | JUMP_IF_POSITIVE,
    # je
    LATCH_PC | MPC_SEL__FETCH | LATCH_MPC | JUMP_IF_ZERO,
    # jne
    LATCH_PC | MPC_SEL__FETCH | LATCH_MPC | JUMP_IF_NEGATIVE | JUMP_IF_POSITIVE,
    # jg
    LATCH_PC | MPC_SEL__FETCH | LATCH_MPC | JUMP_IF_POSITIVE,
    # jl
    LATCH_PC | MPC_SEL__FETCH | LATCH_MPC | JUMP_IF_NEGATIVE,
    # jge
    LATCH_PC | MPC_SEL__FETCH | LATCH_MPC | JUMP_IF_POSITIVE | JUMP_IF_ZERO,
    # jle
    LATCH_PC | MPC_SEL__FETCH | LATCH_MPC | JUMP_IF_NEGATIVE | JUMP_IF_ZERO,
    # in
    INPUT_PORT_READ | ACC_SEL__INPUT_PORT | MPC_SEL__NEXT | LATCH_MPC,
    ACC_SEL__INPUT_PORT | LATCH_ACC | LATCH_PC | MPC_SEL__FETCH | LATCH_MPC,
    # out
    OUTPUT_PORT_WRITE | LATCH_PC | MPC_SEL__FETCH | LATCH_MPC,
...
class Machine:
    ...
    def __init__(self, data, input_buffer, output_buffer, instructions, microcode):
...
def print_status(ticks, dp, cu):
    ...
    if arg_type == "immediate" or opcode.startswith("j") or opcode in ["in", "out"]:
        ...
        arg_repr = ""
    print(
        f"tick #{ticks + 1}: PC={cu.pc.output()} "
        + f"MPC={cu.mpc.output()} ACC={cu.dp.acc.output()} "
        + f"DA={cu.dp.data_address.output()} "
        + f"DOUT={cu.dp.data_memory.output()}  {opcode.upper()} {arg_repr}"
    )
    print(f"input={list(map(chr, dp.input_port.input_buffer))} output={list(map(chr, dp.output_port.output_buffer))}")
